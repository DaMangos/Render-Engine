...
std::vector<std::shared_ptr<vk::raii::ImageView const>> graphics::detail::vulkan::create_image_view(
  ...
  std::shared_ptr<std::tuple<vk::SwapchainCreateInfoKHR, vk::Extent2D, vk::Extent2D> const> const & swapchain_data)
{
  ...
  for(auto const & image : images)
  {
    ...
    image_view.emplace_back(make_shared_with_deleter_data<vk::raii::ImageView const>(std::make_tuple(device, swapchain),
                                                                                     *device,
