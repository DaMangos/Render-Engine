...
constexpr Init inner_product(LhsTuple && lhs, RhsTuple && rhs, Init init, BinaryOp1 op1, BinaryOp2 op2)
{
  ...
  {
    ((init = op1(std::move(init), op2(std::get<I>(std::forward<LhsTuple>(lhs)), std::get<I>(std::forward<RhsTuple>(rhs))))),
     ...);
  }(std::make_index_sequence<std::min(std::tuple_size_v<std::decay_t<LhsTuple>>, std::tuple_size_v<std::decay_t<RhsTuple>>)>());
