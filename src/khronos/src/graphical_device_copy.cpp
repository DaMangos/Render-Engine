...
// [[nodiscard]]
// static int score(vk::raii::PhysicalDevice const & physical_device)
// {
...
// {
//   return render_window{*this, std::move(present_window)};
// }
...
//   auto const physical_devices = library.instance->enumeratePhysicalDevices();
...
//     auto device_extensions = ::get_required_device_extensions(potential_physical_device);

//     if(not device_extensions)
//     {
//       logging::warning() << physical_device_name
...
//     auto const unavailable_features_indices = ::find_unavailable_feature_indices(potential_physical_device,
...
//       if(glfw::default_library.get_physical_device_presentation_support(present_queue_family_index))
//         break;
...
//     if(::score(potential_physical_device) > (physical_device ? ::score(*physical_device) : 0))
//       continue;

//     logging::info() << physical_device_name << "is suitable";

//     auto const physical_device_dependencies = dependency_builder<vk::raii::PhysicalDevice, vk::raii::Instance>{}
//                                                 .add_dependency(library.instance);

//     physical_device.reset(physical_device_dependencies, potential_physical_device);
...
//   auto const device_dependencies = dependency_builder<vk::raii::Device, vk::raii::PhysicalDevice>{}  //
//                                      .add_dependency(physical_device);

//   device.reset(device_dependencies, *physical_device, device_create_info);

//   auto const device_queue_info = vk::DeviceQueueInfo2{}.setQueueFamilyIndex(queue_family_index).setQueueIndex(0);

//   auto const queue_dependencies = dependency_builder<vk::raii::Queue, vk::raii::Device>{}.add_dependency(device);

//   queue.reset(queue_dependencies, *device, device_queue_info);
// }